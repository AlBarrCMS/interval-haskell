...
module Polynomial(
  ...
  -- | term needs a coefficient (even if it's 1) and a power of x (even if it's
  -- | x^0 or x^1)
  ...
  construct_poly string_rep = reduce_full $ Polynomial $ map to_poly_term terms
    where
      to_poly_term term_string = Term (get_coeff term_string)
                                      (get_vars term_string)
  ...
  -- | Construct a polynomial in one arbitrary variable from a list of
  -- | coefficients
  ...
  construct_bivariate_polynomial :: (Num a)
                                 => Char -> Char -> [[a]] -> Polynomial a
  construct_bivariate_polynomial a_var b_var coeffs = Polynomial term_list
    where
      indexed_coeffs =
          map (\(n, lst) -> zip3 lst (repeat n) [0 .. ]) $ zip [0 .. ] coeffs
      ...
      to_term (coeff, a_pow, b_pow) =
          Term coeff [Atom a_var a_pow, Atom b_var b_pow]
  ...
  replace_variable :: (Eq a, Num a, Ord a)
                   ...
                   -> Polynomial a
                   -> Polynomial a
  replace_variable var replacement (Polynomial terms) =
      sum $ map (replace_variable_term var replacement) terms
    where
      replace_variable_term :: (Num a)
                            => Char
                            -> Polynomial a
                            -> Term a
                            -> Polynomial a
  ...
  replace_variable var replacement poly               =
      pmap (replace_variable var replacement) poly

  -- | Evaluate a polynomial with given values for some of its variables
  evaluate :: (Eq a, Num a, Ord a)
           => [Char]
           -> [a]
           -> Polynomial a
           -> Polynomial a
  ...
  evaluate_one_var :: (Eq a, Num a, Ord a)
                   => Char
                   -> a
                   -> Polynomial a
                   -> Polynomial a
      ...
      evaluate_term var val term = Term new_coeff irrelevant_atoms
        where
          ...
          new_coeff = if length relevant_atoms > 0 then
                        ...
                        coeff
  evaluate_one_var var val poly               =
      pmap (evaluate_one_var var val) poly
  ...
  variable_set (Polynomial terms) = Set.unions $ map variables_in_term terms
    where
      ...
      variables_in_term (Term _ atoms) =
          Set.fromList $ map (\(Atom var _) -> var) atoms

  -- | Returns the a list of partial derivatives of a polynomial with respect
  -- to the specified variables. The derivatives are returned in the order the
  ...
  -- | Returns a matrix of second derivatives of a polynomial with respect to
  -- the specified variables. Since polynomials are infinitely differentiable,
  -- the second partial derivatives commute, so this matrix is symmetric (and
  -- thus can be viewed as being in row-major or column-major order). The order
  -- of the derivatives along each dimension of the matrix is the same as the
  ...
  -- | If we write x as (c + (x-c)), we can find the Taylor expansion of p(x)
  -- | by writing p(c + (x-c)) and expanding. We use this technique to find the
  ...
  taylor_expand vars center poly = compose undo_replacements new_poly
    where
      undo_replacements =
          zipWith3 (\temp_var var pt ->
              replace_variable temp_var
                               (Polynomial [Term 1 [Atom var 1],
                                            Term (-pt) []]))
                   temp_vars
                   vars
                   center
  ...
  -- | Return a list of the mixed partial derivatives used in a Taylor
  -- | approximation of degree deg
  taylor_partials :: (Num a, Ord a)
                  => [Char]
                  -> Int
                  -> Polynomial a
                  -> [[Polynomial a]]
  taylor_partials vars deg poly =
      take (deg + 1)
          $ unfoldr (\derivs ->
  ...
  differentiate var (Sum a b)          =
      (differentiate var a) + (differentiate var b)
  differentiate var (Product a b)      =
      ((differentiate var a) * b) + (a * (differentiate var b))
  differentiate var (Power a n)        =
      (scalar_multiply (fromIntegral n) (Power a (n - 1)))
          * (differentiate var a)
  ...
  scalar_multiply s (Sum a b)          =
      (scalar_multiply s a) + (scalar_multiply s b)
  ...
  reduce_full (Power a n)          =
      reduce_full $ foldl' multiply_full a $ take (n-1) $ repeat a
  ...
  add_full a b                           =
      add_full (reduce_full a) (reduce_full b)

  -- | Return the expanded product of polynomials
  multiply_full :: (Num a, Eq a) => Polynomial a -> Polynomial a -> Polynomial a
  multiply_full (Polynomial terms) poly@(Polynomial _) =
      foldl' add 0 $ map (multiply_by_term poly) terms
  multiply_full a b                                    =
      multiply_full (reduce_full a) (reduce_full b)

  -- | Multiply out a polynomial times a term of a polynomial
  multiply_by_term :: (Num a, Eq a) => Polynomial a -> Term a -> Polynomial a
  multiply_by_term (Polynomial terms) term =
      Polynomial (map (multiply_terms term) terms)
  ...
  reduce_sums (Polynomial terms) =
    Polynomial (filter is_nonzero
        $ compress [] (head sorted_reduced_terms) (tail sorted_reduced_terms))
      where
