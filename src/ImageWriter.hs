...
module ImageWriter (
  ...
  -- | An image specified by a width and height (in pixels) and a sequence of
  -- pixel color values
  ...
  write_to_ppm :: (Floating f, RealFrac f)
               => Int     -- ^ The maximum pixel color value
               -> Image f -- ^ The image to print
               -> String  -- ^ The ppm string
  write_to_ppm max_color (Image w h pixels) =
      join ["P3", spaces[w, h], show max_color, colors pixels]
    where
      join_with x = Data.List.concat . Data.List.intersperse x
      join = join_with "\n"
