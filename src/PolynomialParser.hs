...
module PolynomialParser (
  ...
  fix_negative_signs (x:y:zs)
      | x == '-' && not (Char.isDigit y) =
          '-' : '1' : y : fix_negative_signs zs
  ...
  parse_float_polynomial poly_string =
      parse_polynomial poly_string :: Maybe (Polynomial Float)
  ...
  term_parser_nontrivial_coeff :: (Num a, Read a)
                               => Parsec.Parsec String () (Term a)
  term_parser_nontrivial_coeff =
    do
      Parsec.try $
        do
  ...
  term_parser_nontrivial_var :: (Num a, Read a)
                             => Parsec.Parsec String () (Term a)
  term_parser_nontrivial_var =
    do
      Parsec.try $
        do
          coeff <- Parsec.option "1"
              $ Parsec.many1 (Parsec.oneOf "0123456789.-e[]")
