...
naive_necessary_boxes :: (RealFrac a, Floating a, Show a, Num a, Ord a)
                      => Polynomial a -> a -> Int
naive_necessary_boxes poly resolution = trace (show resolution)
    $ length
    $ zeros poly "xy" [Interval (-3) 3, Interval (-3) 3] resolution

rin_necessary_boxes :: (RealFrac a, Floating a, Show a, Num a, Ord a)
                    => Polynomial a -> a -> Int
rin_necessary_boxes poly resolution = trace (show resolution)
    $ length
    $ rin_solve poly
                "xy"
                resolution
...
rin_end_condition poly resolution =
    count_termination_rin_solve poly
                                "xy"
                                resolution
                                resolution
                                resolution
...
-- command line arguments to a ppm or csv file. The first argument is the
-- polynomial as a string and the second is the output file name. The file name
-- must end in either .ppm or .csv
main :: IO ()
main =
  do
    args <- getArgs
    ...
    resolutions <- return
        $ map (\x -> x / 10000000)
        $ map fromIntegral [10000, 9800 .. 10]
    rin_num_boxes <- trace (show resolutions)
        $ return
        $ map (rin_necessary_boxes poly) resolutions
