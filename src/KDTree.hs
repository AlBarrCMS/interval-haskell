...
module KDTree (
  ...
  -- | Constructs a KDTree consisting of a single leaf with the given
  -- boundaries and other value
  ...
  -- non-leaf
  split :: (Num a, Ord a, Fractional a)
        => KDTree a b
        -> (KDTree a b, KDTree a b)
  ...
  split (Leaf boundaries val) = (Leaf lower_part val, Leaf upper_part val)
    where
      ...
      (max_dim_index, _) =
          maximumBy (\(_, a) (_, b) -> compare a b) indexed_widths
      indexed_widths =
          zip [0 .. ] $ map (\(low, high) -> high - low) $ toList boundaries
  ...
  width (Tree _ )           =
      error "KDTree: Tried to get width of non-leaf node"
  width (Leaf boundaries _) =
      Prelude.maximum $ map (\(low, high) -> high - low) $ toList boundaries

  -- | If passed a leaf, sets the leaf's value. Otherwise, throws an error
  set_leaf_val :: KDTree a b -> b -> KDTree a b
  set_leaf_val t@(Tree _) _            =
      error "KDTree: tried to set value of non-leaf node"
  ...
  get_leaf_val (Tree children) =
      error "KDTree: tried to get value of non-leaf node"

  -- | If passed a leaf, returns the a list of the leaf's boundaries.
