module RemainderIntervalNewton (
  ...
  construct_monomial var constant =
  ...
  linearize_equation vars center bounds poly =
    ...
    where
      -- The result of conjugating a matrix (given in row-major order) by a
      -- vector
      conjugate :: (Num a) => [a] -> [[a]] -> a
      conjugate vec mat = vec <.>(map (vec <.>) mat)

      -- The hessian matrix of the polynomial conjugated by the vector (x-c)
      -- where x is the vector of variables and c is the center
      hessian_product = to_const
          $ evaluate vars bounds
          $ (fmap from_num)
          $ conjugate monomials
          $ hessian vars poly
      ...
      -- at a point
      interval_evaluate :: (Num a, Ord a)
                        => [Char] -> [a] -> Polynomial a -> Interval a
      interval_evaluate vars center poly =
          from_num $ to_const $ evaluate vars center poly

      -- Polynomials of the form (x-c) for the corresponding variables (x's)
      -- and center coordinates (c's)
  ...
  crop_equation vars center grad remainder region =
    ...
    where
      -- Monomials of the form (x-c) for the corresponding variables (x's) and
      -- center coordinates (c's) evaluated on the initial region. This is just
      -- found by subtracting the centers from the region components.
      evaluated_monomials = region `vector_minus` (map from_num center)

      -- Range of the inclusion function evaluated on the region
      inclusion =
          remainder + sum (zipWith scalar_mult grad evaluated_monomials)

      -- A list where the ith component is dp/dx_i (c_i) * (/x/_i - c_i)
      -- x_i is the ith variable, /x/_i, is the ith interval of the region, and
      -- c_i is the ith component of the center.
      partial_products = zipWith scalar_mult grad evaluated_monomials

      -- The component-wise inverse interval sum between the partial_products
      -- list and the inclusion function of the polynomial on the region
      numerators = zipWith (<->) (repeat inclusion) partial_products

      -- The component-wise division of the numerators by the gradient of the
      -- polynomial
      inclusion_term = zipWith scalar_div grad numerators

      -- The component-wise interval difference between the center and terms
      -- computed above
  ...
  -- perpindicular to the gradient of a function (these divisions best capture
  -- the variation of the function). This process and the formula for the
  ...
  rin_subdivide grad remainder region = subregions
    where
      ...
      gradient_weighted_widths =
          zipWith (*) (map width region) (map Prelude.abs grad)

      -- The index of the longest side of the region
      max_index =
          snd $ maximumBy (comparing fst) (zip gradient_weighted_widths [0..])

      subdivision_size = (width (region !! max_index))
          / (width remainder)
          * (grad !! max_index)
      ...
      split_points =
          take (num_subdivisions + 1) $ scanl1 (+) (repeat subregion_width)
  ...
  generic_rin_solve bundle_data
    ...
    where
      ...
      subregion_solutions =
          map (generic_rin_solve bundle_data
                                 poly
                                 vars
                                 max_soln_size
                                 max_lin_size
