...
-- | Computes the corner of an interval. If the interval includes 0, the corner is 0. 
-- Otherwise, it is the bound of the interval that is closest to 0.
...
-- | Computes the interval which has the absolute values of the bounds of the input
-- interval as bounds
...
-- | Computes the mignitude of an interval (the minimum element of the absolute value
-- of the interval)
mig :: (Num a, Ord a) => Interval a -> a
mig interval = lower where (Interval lower upper) = Interval.abs interval

-- | Computes the magnitude of an interval (the maximum element of the absolute value
-- of the interval)
mag :: (Num a, Ord a) => Interval a -> a
mag interval = upper where (Interval lower upper) = Interval.abs interval

-- | Computes the sign of an interval (0 if the interval includes 0, -1 if the interval
-- is all below 0, and 1 if the interval is all above 0)
sign :: (Num a, Ord a) => Interval a -> a
sign (Interval lower upper) = if upper <= 0 then -1 else
                              if lower >= 0 then 1 else
                              0
-- | Generates the interval hull of a list of inputs (an interval that contains them all)
hull :: (Num a, Ord a) => [a] -> Interval a
hull lst = Interval (minimum lst) (maximum lst)

-- | Generates an interval whose elements are the negations of the elements of the 
-- input interval
neg :: (Num a) => Interval a -> Interval a
neg (Interval lower upper) = Interval (-upper) (-lower)

-- | Generates an interval whose elements are the sums of elements of the input intervals
...
-- | Generates an interval whose elements are the differences of elements in the input
-- intervals (the elements from the second interval are subttracted from those from the
-- first interval).
...
-- of the second are in the denominator)
divided :: (Num a, Ord a, Fractional a) => Interval a -> Interval a -> Interval a
divided (Interval low_a up_a) (Interval low_b up_b) = 
    hull [low_a / low_b, up_a / low_b, low_a / up_b, up_a / up_b]

-- | Generates the interval whose elements are the powers of elements in the input interval
